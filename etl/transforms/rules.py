...
fallback; see `./CONTEXT.md`). `constant` has a batching rule (rebuild with
the same payload, result unmapped) but NO AD rules: the sweep machinery
handles it structurally (fixed data ⇒ zero tangent, nothing to propagate).
...
`etl.ops.*` functions (two cases need the raw builder because the `ops`
frontends cannot express them: multi-axis `gather` and batched `scatter` —
both go through `trace.current_builder()`, the same builder `ops` resolves).

v1 gaps implemented as explicit `TransformError` (never silent fallback):
`conv` VJP (no transposed-convolution op in the IR), `conv` batching with a
mapped kernel (batch_group_count must be static), `slice` batching with a
mapped operand (the IR slice op requires static int limit_indices, which
cannot express a full-range slice over the symbolic batch dim), and
...
# operand gets leading size-one dims inserted (broadcast) so shapes align.
ELEMENTWISE_OPS = (
...
# Reductions: shift the reduced axis/axes by the number of leading mapped
# axes (sum/mean/prod keep the mapped axes; max/min follow the same shift).
# `argmax`/`argmin` batching lives here too (axis-based, not pointwise).
REDUCTION_OPS = (
...
# operands; AD via transposed dot (conv VJP: TransformError — no transposed
# conv op in the IR).
DOT_OPS = ("dot", "conv")

# Triangular/cumulative/solve: tril/triu/cumsum pass mapped axes through
# (per-row semantics); solve broadcasts batch dims like dot.
...
def _zero_of(value: ir.Value) -> ir.Value:
    """Emit an all-zeros tensor with `value`'s shape/dtype.

    Integer 0 keeps weak promotion dtype-exact for float/complex operands
    (the differentiable case); integer/bool primals would promote, so those
    get an explicit cast back.
    """
...
def _conv_kwargs(op: ir.Op) -> dict:
    ...
    if attrs.get("batch_group_count", 1) != 1:
        raise TransformError(
            "vectorize/jvp: cannot transform 'conv' with batch_group_count "
            f"{attrs['batch_group_count']!r} (v1 gap: batch-grouped conv)"
        )
    return dict(
...
def _gelu_coeff(x: "core.SymbolicTensor") -> "core.SymbolicTensor":
    ...
    term1 = ops.multiply(
        _c(x.dtype, 0.5), ops.add(_c(x.dtype, 1.0), ops.erf(u))
    )
    term2 = ops.multiply(
        ops.multiply(x, _c(x.dtype, 0.3989422804014327)),
        ops.exp(ops.negate(ops.square(u))),
    )
...
def _pointwise_batching(op, operands, axes):
    """Elementwise batching: align unmapped operands with leading size-one
    dims (broadcast), rebuild the op, result axes = union of operand axes."""
    name = op.name
    ...
    for value, count in zip(operands, counts):
        if count < mapped_count:
            value = ops.broadcast(
                _sym(value), (1,) * (mapped_count - count) + value.type.shape
            ).value
        values.append(value)
    if name == "cast":
        out = ops.cast(_sym(values[0]), op.attributes["dtype"])
    elif name == "broadcast":
        # The mapped dims must be preserved: new target = operand batch dims
        # + the original target shape.
        target = tuple(op.attributes["shape"])
        out = ops.broadcast(
            _sym(values[0]), values[0].type.shape[:mapped_count] + target
        )
    elif name == "select":
    ...
    return (out.value,), (result_axes,)


def _reduction_batching(op, operands, axes):
    """Reduction batching: shift the reduced axis/axes by the mapped count;
    the result keeps the operand's mapped axes. Also handles `argmax`/
    `argmin` (single `axis` attr; axis=None flattens the unvectorized dims
    via a reshape)."""
    value = operands[0]
    mapped = axes[0]
    count = mapped.count
    ...
    if name in ("argmax", "argmin"):
        ...
        if axis is None and count > 0:
            # Flatten-over-all per batch row: reshape (batch..., *dims) →
            # (batch..., prod(dims)), argmax the last axis.
            old_shape = op.operands[0].type.shape
            flat = _fold_product(old_shape)
            if flat is None:
                raise TransformError(
                    f"vectorize: cannot batch '{name}' with axis=None over "
                    "dynamic dims (v1 gap)"
                )
            flat_value = ops.reshape(
                _sym(value), value.type.shape[:count] + (flat,)
            )
            out = fn(flat_value, axis=count, keepdims=False)
            if keepdims:
                out = ops.reshape(
                    out, value.type.shape[:count] + (1,) * len(old_shape)
                )
        else:
    ...
    if reduced:
        ...
        # Reduce-all over a mapped scalar input: identity per row, with the
        # reduction's dtype rule applied (defensive; mapping rank-0 inputs
        # is normally rejected by vectorize).
        ...
        # Empty axes = ALL (unvectorized) axes: reduce over the per-row axes
        # (mapped dims excluded).
        new_axes = tuple(range(count, count + old_rank))
    out = fn(
        _sym(value), axes=new_axes, keepdims=attrs["keepdims"]
    )
    return (out.value,), (mapped,)


def _reshape_batching(op, operands, axes):
    """Reshape batching: the mapped dims must be preserved — new target =
    operand batch dims + the original target shape (the -1 wildcard still
    resolves: the batch factors cancel in the element-count division)."""
    value = operands[0]
    mapped = axes[0]
    target = tuple(op.attributes["shape"])
    if mapped.count > 0:
...
def _transpose_batching(op, operands, axes):
    """Transpose batching: mapped leading axes stay leading; the original
    permutation (None = full reversal) applies to the unvectorized axes."""
    value = operands[0]
    mapped = axes[0]
    count = mapped.count
    if count == 0:
...
def _concatenate_batching(op, operands, axes):
    """Concatenate batching: broadcast operands missing mapped axes up to the
    batch dims (numpy concat does NOT broadcast — the batch dims must agree
    explicitly), then shift the axis by the mapped count."""
    counts = [ax.count for ax in axes]
    mapped_count = max(counts) if counts else 0
    axis = op.attributes["axis"]
    if mapped_count == 0:
    ...
    for value, count in zip(operands, counts):
        if count < mapped_count:
            value = ops.broadcast(
                _sym(value), tuple(batch_dims) + value.type.shape
            ).value
...
def _structural_batching(op, operands, axes):
    ...
    name = op.name
    if name == "slice":
        return _slice_batching(op, operands, axes)
    if name == "pad":
        return _pad_batching(op, operands, axes)
    return _concatenate_batching(op, operands, axes)


def _gather_batching(op, operands, axes):
    """Gather batching: remove ALL mapped axes plus the data axis with one
    multi-axis gather (infer_gather splices ``indices.shape`` wholesale at
    the smallest removed axis — the indices' batch dims then double as the
    removed data batch axes, which is exactly the per-row semantics), then
    one transpose moves the index dims into the data axis's position and a
    reshape restores the original (possibly multi-dim) index shape. Mapped
    data with unmapped indices is a v1 TransformError."""
    x, indices = operands
    kx, ki = axes[0].count, axes[1].count
    attr_axes = op.attributes["axes"]
    if len(attr_axes) != 1:
        raise TransformError(
            "vectorize: cannot batch op 'gather' with multiple axes (v1 gap)"
        )
    axis = attr_axes[0]
    if kx == 0 and ki == 0:
    ...
    if kx > 0 and ki != kx:
        raise TransformError(
            "vectorize: cannot batch op 'gather' with mapped data but "
            "unmapped indices (v1 deferral: index broadcasting is not "
            "supported)"
        )
    ...
    if kx < mapped_count:
        ...
        x = ops.broadcast(
            _sym(x), (1,) * (mapped_count - kx) + original_shape
        ).value
        x = ops.broadcast(_sym(x), tuple(batch_dims) + original_shape).value
    # Merge the index dims (beyond the batch dims) into one axis so the
    # indices rank matches the removed-axes count (mapped + 1); a
    # batch-only index shape (B,) becomes (B, 1).
    index_dims = indices.type.shape[mapped_count:]
    merged_index = _fold_product(index_dims)
    if merged_index is None:
        raise TransformError(
            "vectorize: cannot batch op 'gather' with dynamic index dims "
            "(v1 gap)"
        )
    if merged_index != 1 or index_dims == ():
        indices = ops.reshape(
            _sym(indices), indices.type.shape[:mapped_count] + (merged_index,)
        ).value
    ...
    gathered = builder.create(
        "gather",
        operands=(x, indices),
        attributes={"axes": removed},
    ).results[0]
    # gathered = (batch..., J, prefix_dims..., suffix_dims...) — move J into
    # the data axis's position: (batch..., prefix_dims..., J, suffix_dims...).
    n_prefix = axis
    rank = gathered.type.rank
    perm = (
        list(range(mapped_count))
        + list(range(mapped_count + 1, mapped_count + 1 + n_prefix))
        + [mapped_count]
        + list(range(mapped_count + 1 + n_prefix, rank))
    )
    reordered = ops.transpose(_sym(gathered), axes=tuple(perm)).value
    # Restore the original multi-dim index shape (and drop the size-one
    # J axis for batch-only indices).
    target = (
        tuple(reordered.type.shape[:mapped_count])
        + tuple(original_shape[:axis])
        + tuple(index_dims)
        + tuple(original_shape[axis + 1:])
    )
...
def _scatter_batching(op, operands, axes):
    """Scatter batching: shift the axis by the mapped count; the data operand
    is broadcast up to the batch dims when unmapped. Batched scatter cannot
    go through `ops.scatter` (its frontend rank check rejects batch dims),
    so the op is built on the raw builder."""
    ...
    if kx > 0 and (ki != kx or ku != kx):
        raise TransformError(
            "vectorize: cannot batch op 'scatter' when the data operand is "
            "mapped but indices/updates are not (v1 deferral)"
        )
    if kx == 0 and ki != ku:
        raise TransformError(
            "vectorize: cannot batch op 'scatter' when indices and updates "
            "disagree on mapped axes (v1 deferral)"
        )
    if kx < mapped_count:
    ...
    scattered = builder.create(
        "scatter",
        operands=(x, indices, updates),
        attributes={"axis": axis + mapped_count},
    ).results[0]
...
def _gather_scatter_batching(op, operands, axes):
    ...
    if op.name == "gather":
        return _gather_batching(op, operands, axes)
    return _scatter_batching(op, operands, axes)


def _dot_batching(op, operands, axes):
    """Dot batching: matmul already broadcasts batch dims; align unmapped
    operands with leading size-one dims and rebuild."""
...
def _conv_batching(op, operands, axes):
    """Conv batching: the IR conv op has exactly ONE batch dim (sx[0]), so a
    mapped input's batch dim is folded into it via reshape (per-row convs are
    independent — numerically exact). A mapped kernel would need a symbolic
    batch_group_count (static int attr) → v1 TransformError."""
    x, w = operands
    kx, kw = axes[0].count, axes[1].count
    if kw > 0:
        raise TransformError(
            "vectorize: cannot batch op 'conv' with a mapped kernel — "
            "batch_group_count must be a static int (v1 gap)"
        )
    ...
    if old_shape[0] is None:
        raise TransformError(
            "vectorize: cannot batch op 'conv' over a dynamic batch dim "
            "(v1 gap)"
        )
    ...
    back = ops.reshape(
        out, tuple(batch_dims) + (old_shape[0],) + out.shape[1:]
    )
...
def _dot_conv_batching(op, operands, axes):
    ...
    if op.name == "dot":
        return _dot_batching(op, operands, axes)
    return _conv_batching(op, operands, axes)


def _triangular_batching(op, operands, axes):
    """tril/triu batching: the mask applies to the last two dims of every
    batch row — rebuild on the (possibly mapped) operand unchanged."""
    out = getattr(ops, op.name)(_sym(operands[0]), k=op.attributes["k"])
    return (out.value,), (axes[0],)


def _cumsum_batching(op, operands, axes):
    """cumsum batching: shift the scan axis by the mapped count (the scan
    runs per batch row); shape preserved, mapped axes pass through."""
    out = ops.cumsum(
        _sym(operands[0]),
        axis=op.attributes["axis"] + axes[0].count,
...
def _solve_batching(op, operands, axes):
    """Solve batching: infer_solve broadcasts batch dims — align unmapped
    operands with leading size-one dims and rebuild."""
    if op.attributes.get("left_side", True) is not True:
        raise TransformError(
            "vectorize: cannot batch op 'solve' with left_side=False (v1 gap)"
        )
    mapped_count = max(axes[0].count, axes[1].count)
    values = []
    for value, ax in zip(operands, axes):
...
def _constant_batching(op, operands, axes):
    """Constant batching: rebuild with the same payload; the result is
    UNMAPPED (the data is shared across the batch — no mapped dims)."""
...
#: Unary pointwise derivative coefficient d y/d x — self-adjoint elementwise
#: ops share one table between JVP and VJP: fn(a, y) -> SymbolicTensor.
_UNARY_DERIV = {
    "abs": lambda a, y: ops.sign(a),
    "negate": lambda a, y: ops.negate(_c(a.dtype, 1.0)),
    "square": lambda a, y: ops.multiply(_c(a.dtype, 2.0), a),
    "sqrt": lambda a, y: ops.divide(_c(a.dtype, 0.5), y),
    "exp": lambda a, y: y,
    "log": lambda a, y: ops.divide(_c(a.dtype, 1.0), a),
    "log1p": lambda a, y: ops.divide(
        _c(a.dtype, 1.0), ops.add(_c(a.dtype, 1.0), a)
    ),
    "sin": lambda a, y: ops.cos(a),
    "cos": lambda a, y: ops.negate(ops.sin(a)),
    "tan": lambda a, y: ops.add(_c(a.dtype, 1.0), ops.square(y)),
    "tanh": lambda a, y: ops.subtract(_c(a.dtype, 1.0), ops.square(y)),
    "sigmoid": lambda a, y: ops.multiply(
        y, ops.subtract(_c(a.dtype, 1.0), y)
    ),
    "gelu": lambda a, y: _gelu_coeff(a),
    "erf": lambda a, y: ops.multiply(
        _c(a.dtype, 1.1283791670955126), ops.exp(ops.negate(ops.square(a)))
    ),
}
...
def _jvp_subtract(op, tangents):
    ...
    if ta is None and tb is None:
        return (ZeroTangent(),)
    if ta is None:
        return (ops.negate(_sym(tb)).value,)
    if tb is None:
        return (ta,)
...
def _jvp_multiply(op, tangents):
    ...
    terms = []
    if tb is not None:
        terms.append(ops.multiply(_sym(a), _sym(tb)).value)
    if ta is not None:
        terms.append(ops.multiply(_sym(ta), _sym(b)).value)
    if not terms:
        return (ZeroTangent(),)
...
def _jvp_divide(op, tangents):
    ...
    terms = []
    if ta is not None:
        terms.append(ops.divide(_sym(ta), _sym(b)).value)
    if tb is not None:
        terms.append(
            ops.negate(
                ops.divide(ops.multiply(_sym(y), _sym(tb)), _sym(b))
            ).value
        )
    if not terms:
        return (ZeroTangent(),)
...
def _jvp_power(op, tangents):
    ...
    if tb is not None:
        terms.append(
            ops.multiply(_sym(y), ops.multiply(_sym(tb), ops.log(_sym(a)))).value
        )
    if ta is not None:
        terms.append(
            ops.multiply(
                _sym(y), ops.divide(ops.multiply(_sym(b), _sym(ta)), _sym(a))
            ).value
        )
    if not terms:
        return (ZeroTangent(),)
...
def _jvp_relu(op, tangents):
    ...
    return (
        ops.select(
            ops.greater(_sym(a), 0), _sym(ta), _sym(_zero_of(a))
        ).value,
    )
...
_POINTWISE_JVP = {
    "add": _jvp_add,
    "subtract": _jvp_subtract,
    "multiply": _jvp_multiply,
    "divide": _jvp_divide,
    "power": _jvp_power,
    "remainder": _jvp_remainder,
    "maximum": _jvp_maxmin,
    "minimum": _jvp_maxmin,
    "relu": _jvp_relu,
    "cast": _jvp_cast,
...
def _pointwise_jvp(op, tangents):
    """Per-elementwise-op linearization: unary ops use the shared derivative
    table; special cases (product/quotient rules, select on the primal mask,
    cast, broadcast) use their own rules. Primals read from `op.operands`."""
    name = op.name
...
def _vjp_multiply(op, ct, primals):
    ...
    a, b = primals
    return (
        ops.multiply(_sym(ct), _sym(b)).value,
        ops.multiply(_sym(ct), _sym(a)).value,
    )
...
def _vjp_divide(op, ct, primals):
    ...
    g_b = ops.negate(
        ops.divide(ops.multiply(_sym(ct), _sym(y)), _sym(b))
    ).value
    return (g_a, g_b)
...
def _vjp_power(op, ct, primals):
    ...
    y = op.results[0]
    g_a = ops.multiply(
        ops.multiply(_sym(ct), _sym(y)), ops.divide(_sym(b), _sym(a))
    ).value
    g_b = ops.multiply(
        ops.multiply(_sym(ct), _sym(y)), ops.log(_sym(a))
    ).value
    return (g_a, g_b)
...
def _vjp_maxmin(op, ct, primals):
    ...
    return (
        ops.multiply(_sym(ct), mask).value,
        ops.multiply(_sym(ct), ops.subtract(one, mask)).value,
    )
...
def _vjp_broadcast(op, ct, primals):
    """Broadcast VJP: reduce the cotangent over the leading inserted axes and
    over any size-one expanded dims, then reshape back to the input shape."""
    ct = _ok(ct)
    if ct is None:
        return (ZeroTangent(),)
    a = primals[0]
...
_POINTWISE_VJP = {
    "add": _vjp_add,
    "subtract": _vjp_subtract,
    "multiply": _vjp_multiply,
    "divide": _vjp_divide,
    "power": _vjp_power,
    "remainder": _vjp_remainder,
    "maximum": _vjp_maxmin,
    "minimum": _vjp_maxmin,
    "relu": _vjp_relu,
    "cast": _vjp_cast,
...
def _pointwise_vjp(op, cotangents, primals):
    """Per-elementwise-op pullback: product with the local partials (unary
    ops share the self-adjoint derivative table; special cases use their own
    rules); returns a tuple aligned with `op.operands`."""
    name = op.name
    if name in _UNARY_DERIV:
...
def _expand_reduced(v: ir.Value, x: ir.Value, axes, keepdims: bool) -> ir.Value:
    """Expand a reduced value back to the input shape: insert size-one dims
    at the reduced axes (when not kept) and broadcast to the input shape."""
    x_shape = tuple(x.type.shape)
    if v.type.rank != len(x_shape):
        v = ops.reshape(
            _sym(v), tuple(1 if i in axes else d for i, d in enumerate(x_shape))
        ).value
...
def _reduction_vjp(op, cotangents, primals):
    """Reduction VJP: broadcast the cotangent back to the input shape, with
    the reduction-specific weighting (mean divides by the static reduction
    count; prod multiplies by y/x; max/min select on the arg mask)."""
    ct = _ok(cotangents[0])
    if ct is None:
        return (ZeroTangent(),)
    x = primals[0]
    kind = op.attributes["reduce_op"]
    axes = tuple(op.attributes["axes"])
    keepdims = op.attributes["keepdims"]
    rank = x.type.rank
    eff = axes if axes else tuple(range(rank))
    if not eff:
        return (ct,)  # rank-0 identity
    if kind == "sum":
    ...
    if kind == "mean":
        ...
        for i in eff:
            ...
            if not isinstance(dim, int):
                raise TransformError(
                    f"grad/vjp: cannot differentiate 'mean' over symbolic "
                    f"dim {dim!r} — the reduction count is unknown (v1 gap)"
                )
            count *= dim
...
def _structural_jvp(op, tangents):
    """Structural JVP: apply the same structural op to the tangents (all are
    linear in their tensor operands)."""
    name = op.name
    if name == "reshape":
        t = _ok(tangents[0])
        if t is None:
    ...
    if name == "slice":
        ...
        return (
            ops.slice(
                _sym(t), starts, lengths, strides=op.attributes["strides"] or 1
            ).value,
        )
    if name == "pad":
        t = _ok(tangents[0])
    ...
    if name == "concatenate":
        ...
        if all(t is None for t in ts):
            return (ZeroTangent(),)
        aligned = [
            _sym(t if t is not None else _zero_of(p))
            for t, p in zip(ts, op.operands)
        ]
    ...
    if name == "gather":
        ...
        if len(axes) != 1:
            raise TransformError(
                "jvp: cannot differentiate 'gather' with multiple axes (v1 gap)"
            )
    ...
    if tx is None and tupd is None:
        return (ZeroTangent(),)
    x = tx if tx is not None else _zero_of(op.operands[0])
    upd = tupd if tupd is not None else _zero_of(op.operands[2])
    return (
        ops.scatter(
            _sym(x), _sym(op.operands[1]), _sym(upd), axis=op.attributes["axis"]
        ).value,
    )


def _slice_vjp(op, ct, x):
    """Slice VJP: stride-1 slices restore via pad ((start, dim-limit) per
    sliced axis); a single strided axis restores via scatter with a static
    arange index; anything else is a v1 TransformError."""
    starts = op.attributes["start_indices"]
    limits = op.attributes["limit_indices"]
    strides = tuple(
        op.attributes["strides"]
        if op.attributes["strides"] is not None
        else (1,) * len(starts)
    )
    rank = x.type.rank
    partial = [
    ...
    if not partial:
        return ct  # full slice: gradient is the identity
    ...
    if not strided:
        ...
        for i in range(rank):
            ...
            if isinstance(dim, int):
                ...
                raise TransformError(
                    f"grad/vjp: cannot invert a slice over symbolic dim "
                    f"{dim!r} (v1 gap)"
                )
    ...
    if len(partial) != 1 or len(strided) != 1:
        raise TransformError(
            "grad/vjp: cannot invert a slice with strided entries over "
            "multiple axes (v1 gap)"
        )
    axis = partial[0]
    zeros = _zero_of(x)
    indices = ops.constant(
        core.tensor(np.arange(starts[axis], limits[axis], strides[axis]))
    )
    return ops.scatter(_sym(zeros), indices, _sym(ct), axis=axis).value


def _structural_vjp(op, cotangents, primals):
    """Structural VJP: the inverse structural op applied to the cotangent
    (reshape back, inverse permutation, pad for stride-1 slices / scatter
    for one strided axis, inverse slice for pad, per-segment slices for
    concatenate, scatter for gather, gather for scatter)."""
    ct = _ok(cotangents[0])
    if ct is None:
        return (ZeroTangent(),) * len(primals)
    name = op.name
    x = primals[0]
    if name == "reshape":
    ...
    if name == "pad":
        ...
        for dim in x.type.shape:
            if not isinstance(dim, int):
                raise TransformError(
                    f"grad/vjp: cannot invert a pad over symbolic dim "
                    f"{dim!r} (the inverse slice needs static lengths, v1 gap)"
                )
            lengths.append(dim)
    ...
    if name == "concatenate":
        ...
        for p in primals:
            ...
            if not isinstance(length, int):
                raise TransformError(
                    f"grad/vjp: cannot split a concatenate over symbolic "
                    f"dim {length!r} (v1 gap)"
                )
            ...
            if any(not isinstance(d, int) for d in lens):
                raise TransformError(
                    "grad/vjp: cannot split a concatenate with symbolic "
                    f"dims {tuple(ct.type.shape)!r} (v1 gap)"
                )
            grads.append(
                ops.slice(_sym(ct), tuple(starts), tuple(lens), strides=1).value
            )
            offset += length
        return tuple(grads)
    if name == "gather":
        ...
        if len(axes) != 1:
            raise TransformError(
                "grad/vjp: cannot differentiate 'gather' with multiple axes "
                "(v1 gap)"
            )
        g = ops.scatter(
            _sym(_zero_of(x)), _sym(primals[1]), _sym(ct), axis=axes[0]
        )
        return (g.value, ZeroTangent())
    # scatter: g_x = scatter(ct, idx, zeros-of-updates); g_updates =
    # gather(ct, idx); indices are non-differentiable.
    zeros_upd = _zero_of(primals[2])
    g_x = ops.scatter(
        _sym(ct), _sym(primals[1]), _sym(zeros_upd), axis=op.attributes["axis"]
    )
    g_upd = ops.gather(
        _sym(ct), _sym(primals[1]), axis=op.attributes["axis"]
    )
    return (g_x.value, ZeroTangent(), g_upd.value)


def _dot_conv_jvp(op, tangents):
    """Dot/conv JVP: the two product terms (dot(a,tb)+dot(ta,b) /
    conv(a,tw)+conv(ta,w)), zero tangents skipped."""
    ta, tb = _ok(tangents[0]), _ok(tangents[1])
    terms = []
    if op.name == "dot":
        if tb is not None:
            terms.append(ops.dot(_sym(op.operands[0]), _sym(tb)).value)
        if ta is not None:
            terms.append(ops.dot(_sym(ta), _sym(op.operands[1])).value)
    else:  # conv
        kwargs = _conv_kwargs(op)
        if tb is not None:
            terms.append(ops.conv(_sym(op.operands[0]), _sym(tb), **kwargs).value)
        if ta is not None:
            terms.append(ops.conv(_sym(ta), _sym(op.operands[1]), **kwargs).value)
    if not terms:
        return (ZeroTangent(),)
    return (_sum_terms(terms),)
...
def _dot_vjp(op, ct, primals):
    """Dot VJP: g_a = dot(ct, bᵀ), g_b = dot(aᵀ, ct), broadcast-only leading
    batch dims summed away."""
    ct = _ok(ct)
    if ct is None:
        return (ZeroTangent(), ZeroTangent())
    a, b = primals
    ...
    b_t = ops.transpose(
        _sym(b), axes=tuple(range(rb - 2)) + (rb - 1, rb - 2)
    )
    g_a = ops.dot(_sym(ct), b_t).value
    a_t = ops.transpose(
        _sym(a), axes=tuple(range(ra - 2)) + (ra - 1, ra - 2)
    )
...
def _dot_conv_vjp(op, cotangents, primals):
    """Dot/conv VJP: transposed dot of the cotangent with each primal
    operand; conv has NO transposed form in the IR → TransformError."""
    if op.name == "conv":
        raise TransformError(
            "grad/vjp: no VJP rule for op 'conv' — the IR has no "
            "transposed-convolution op (v1 gap)"
        )
...
def _cumsum_jvp(op, tangents):
    ...
    return (
        ops.cumsum(
            _sym(t), axis=op.attributes["axis"], reverse=op.attributes["reverse"]
        ).value,
    )
...
def _cumsum_vjp(op, cotangents, primals):
    ...
    return (
        ops.cumsum(
            _sym(ct),
            axis=op.attributes["axis"],
            reverse=not op.attributes["reverse"],
        ).value,
    )


def _solve_jvp(op, tangents):
    """Solve JVP: a·dx + da·x = db ⇒ dx = solve(a, db − da·x). Vector b
    (rank 1) goes through a (n, 1) reshape (matvec via dot)."""
    if op.attributes.get("left_side", True) is not True:
        raise TransformError(
            "jvp: cannot differentiate op 'solve' with left_side=False "
            "(v1 gap)"
        )
    ta, tb = _ok(tangents[0]), _ok(tangents[1])
    if ta is None and tb is None:
    ...
    if x.type.rank == 1:
        ...
        diff = (
            ops.reshape(_sym(tb), (n, 1)).value
            if tb is not None
            else ops.reshape(_sym(_zero_of(b)), (n, 1)).value
        )
        if ta is not None:
...
def _solve_vjp(op, cotangents, primals):
    """Solve VJP: g_b = solve(aᵀ, ct); g_a = −g_b·xᵀ (outer product via
    reshaped multiply for vector x, dot for matrix x); broadcast-only leading
    batch dims summed away."""
    if op.attributes.get("left_side", True) is not True:
        raise TransformError(
            "grad/vjp: cannot differentiate op 'solve' with left_side=False "
            "(v1 gap)"
        )
    ct = _ok(cotangents[0])
    if ct is None:
    ...
    if x.type.rank == 1:
        ...
        rx = x.type.rank
        x_t = ops.transpose(
            _sym(x), axes=tuple(range(rx - 2)) + (rx - 1, rx - 2)
        )
...
def register_builtin_rules() -> None:
    ...
    `etl/block`). `constant` gets a batching rule only — the AD machinery
    handles it structurally.
    """
    # batching
    nondiff_pointwise = tuple(
        n for n in NONDIFFERENTIABLE_OUTPUT_OPS if n not in ("argmax", "argmin")
    )
    ...
    for name in ("tril", "triu"):
        register_batching_rule(name, _triangular_batching)
    register_batching_rule("cumsum", _cumsum_batching)
    register_batching_rule("solve", _solve_batching)
    ...
    for name in ("tril", "triu"):
        register_jvp_rule(name, _triangular_jvp)
    register_jvp_rule("cumsum", _cumsum_jvp)
    register_jvp_rule("solve", _solve_jvp)
    ...
    for name in ("tril", "triu"):
        register_vjp_rule(name, _triangular_vjp)
    register_vjp_rule("cumsum", _cumsum_vjp)
    register_vjp_rule("solve", _solve_vjp)
