...
- **`block_call` ops do not carry the declared `batching_policy`** (attrs are only `block_name`/`static_args`/`result_specs`; `../block/` also does not pre-register rules for safe policies). The transforms side supports both channels already: an `elementwise`/`map_over_batch` `batching_policy` attribute on the op selects the built-in pass-through (`batching.block_call_pass_through_rule()`), and `../block/` may pre-register that factory's product via `register_batching_rule`. Until one of those lands, `tests/block/test_rules.py::test_elementwise_policy_passes_batch_dims_through` stays red (coordination with `../block/` + `../ir/` — escalate writes to the parent).
- **`tests/block/test_rules.py::test_jvp_derived_from_portable_vjp_fallback` applies `etl.jvp` to `rule_portgrad_loss` (a `sum` over the block ⇒ scalar primal/tangent) but asserts the UNsummed block outputs** (`swish(x)`, `swish'(x)·dx`). The engine is correct (verified numerically; jvp of the plain block fn produces exactly the asserted values) — the test needs a fn swap on the tests side (e.g. wrap the block directly instead of the loss). Do NOT weaken the engine to match the test.
...
- `batching.block_call_pass_through_rule()` is the reusable built-in batching rule for elementwise/map_over_batch block calls — `../block/` may pre-register its product under `block:<name>` for blocks declared with those policies (block-side coordination; transforms never imports block). It rebuilds the `block_call` with `result_specs` == result types exactly (both must be the same `ir.ValueType` tuple — `ir.verify` enforces this), batch dims taken from the operand with the most mapped axes.
