...
- **Nested vmap batch-dim naming**: each `vectorize` pass creates ONE fresh `Dim` named `batch` shared by all its mapped inputs; nested passes create their own fresh objects, also named `batch`. Single transforms are per contract, but don't rely on dim NAMES to identify nesting levels — `vmap`'s output-mappedness detection uses Dim object identity (from the mapped input specs), not names. **Consequence (Defect B, open)**: because every pass reuses the NAME `batch` and the numpy interpreter binds symbolic dims by name, nested vmap with UNEQUAL outer/inner batch extents fails at runtime with `ShapeError: dimension 'batch' is already bound to <outer>, got <inner>` — a valid graph rejected (equal extents pass). A complete fix needs level-distinct dim names (keeping `_BATCH_DIM_NAME` matching) and/or a finer-than-name binding key in `../backends/numpy/interpreter.py` (backend-side, out of scope here).
- **Mapped inputs of one pass share one runtime batch extent (intended)**: the shared `Dim("batch")` makes the numpy interpreter's name-keyed input binding the within-level guard — a mismatched mapped extent raises a clear `ShapeError` ("dimension 'batch' is already bound to 4, got 1") instead of the silent numpy-style broadcasting that distinct per-input dims (`batch`/`batch_1`) used to allow.
