...
- **`backends`** exports: `Backend` ABC — `name: str`, `capabilities: Capabilities` (fields: `dynamic_shapes`, `dtypes`, `collectives`, `runtime_calls`, `custom_blocks`, `async_collectives`), `lower(graph, options) -> LoweredProgram`, `compile(lowered, options) -> CompiledArtifact`, `load(artifact, device) -> Executable`; backend `Executable` protocol — `run(flat_input_tensors) -> flat_output_tensors`, `.functions`, `.device`, optional `.save(path)/.load` (must save artifact + reconstruct explicitly if handles are device-specific). `register/get(name)`, `numpy_backend`, `stablehlo` (export function; `LoweredProgram` for numpy backend wraps serializable program; for stablehlo wraps MLIR text — `.text()`).
...
- **`pipeline`** (`etl/pipeline.py`) exports: `LoweredProgram` (`.text()`, `.save/.load`, `.backend`), `CompiledArtifact` (`.save/.load`, `.backend`, `.signature`), `Executable` (wraps backend executable + Graph tree info: `.run` is via `etl.run`; `.functions`, `.device`, `.save/.load`), `lower/compile/load/run/bind/build/evaluate`.
