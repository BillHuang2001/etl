...
- **`core`** exports: `Tensor`, `SymbolicTensor`, `TensorSpec`, `Dim`, `DimExpr`, `Device`, `TreeSpec` (`flatten`/`unflatten`/`register_pytree_node`), `dtype(obj)`, dtype constants, `ETLError` + subclasses (incl. `VerificationError`), concrete creators, device helpers, `register_operator_handlers(kind, handler)` — a hook dict (`add`/`sub`/`mul`/`matmul`/`getitem`/`truediv`/`pow`/`neg`/`lt`/`le`/`gt`/`ge`/`eq`) that `ops` populates at import time so `SymbolicTensor.__add__` etc. work without import cycles — and `register_constant_builder` (the `etl.constant` hook, populated by `ops`).
