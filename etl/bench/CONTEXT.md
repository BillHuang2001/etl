...
- **IREE adapter ignores `target_backends`**: the adapter's `compile` currently hardcodes `target_backends=["llvm-cpu"]` and does not consume options passed by the harness — the passthrough is accepted silently. Wiring it is an adapter-side change (`etl/backends/adapters/iree.py`, outside this node).
