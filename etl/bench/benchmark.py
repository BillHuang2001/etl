...
import etl
from ._util import best_time_ms, resolve_examples, resolve_torch_mode
...
def benchmark(examples=None, *, use_torch=None, repeats=20, warmup=2,
    ...
    after ``warmup`` untimed runs — the etl graph on the default numpy
    backend, the pure-numpy reference, and (when torch is available /
    requested) the torch reference.
    ...
    Raises:
        ValueError: invalid ``repeats``/``warmup``.
        ...
        try:
            ...
            if enabled:
                ...
                torch_ms = best_time_ms(
                    lambda: example.torch_ref(inputs), warmup, repeats
                )
            speedup_vs_numpy = (
