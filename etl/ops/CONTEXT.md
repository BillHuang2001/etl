...
- `constant` warns (UserWarning) above `ETL_LARGE_CONSTANT_BYTES` (default 1 MiB, env-tunable) and SNAPSHOTS (copies) data; **`constant` requires HOST data — a non-cpu tensor raises `core.DeviceError` at snapshot time** (no implicit device-to-host transfer at snapshot; transfer explicitly via `t.to(core.Device("cpu", 0))` first); `runtime_call` carries the callback as an op attr with effect `callback` — backends may reject it (`BackendError`) but never silently drop/reorder it; `external_call` carries the stable kernel `name` string + `result_specs` (effect `callback`, same rule) and is dispatched by name through `etl.external` at run time — the kernel need NOT be registered at trace time (run-time concern); `stop_gradient` is effect `pure` but transform layers MUST process it before any constant-folding (gradient barrier).
