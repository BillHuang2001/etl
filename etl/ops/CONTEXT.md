...
- **`abs` of complex input (repo bug #2; pinned by `test_abs_complex_numerics_give_real_magnitude`):** the post-cast composition in `elementwise.abs` (abs → cast to the real magnitude dtype) cannot make the graph runnable as-is: `etl.ir` registers `abs` with the dtype-preserving `infer_elementwise_unary`, so the abs op's declared result for a complex operand stays complex, while the numpy backend kernel (`np.abs`) already returns the REAL magnitude dtype (float32/float64) — the interpreter validates kernel outputs against declared types exactly, so running the graph raises `BackendError` on the abs op before the cast ever executes. NO `ops`-level composition can fix this (verified: `ir.verify` enforces shape_fn agreement so explicit `result_types` cannot override the declared dtype; the op registry is owned by `ir` and `ops` must not override it; no real/imag/conj ops exist for a decomposition). The fix belongs in `etl/ir`: a dedicated `infer_abs` hook (complex64 → float32, complex128 → float64, other dtypes preserved) registered for `abs` in `etl/ir/op_defs/elementwise.py` (keep every other unary op on `infer_elementwise_unary`). With that hook landed, the frontend post-cast becomes a transparent no-op and MUST be kept until `test_abs_complex_gives_real_magnitude_dtype` (green, asserts exactly one cast op) is updated tests-side.
