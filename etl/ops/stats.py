...
from . import _utils
from .comparison import greater, not_equal, select
...
def _median_along_axis(x, axis, keepdims) -> "core.SymbolicTensor":
    ...
    result = reduce_mean(middle, axes=(axis,), keepdims=keepdims)
    # numpy median propagates NaN: if any element along the axis is NaN the
    # median is NaN (the sort composition alone would place NaN at the end
    # and return a real middle value).
    nan_count = reduce_sum(
        cast(isnan(x), core.float64), axes=(axis,), keepdims=keepdims
    )
    has_nan = greater(nan_count, 0.0)
    zeros = broadcast(0.0, result.shape)
    return select(has_nan, divide(zeros, zeros), result)
