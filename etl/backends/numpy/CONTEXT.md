...
- **Sparse kernels (16 ops, `kernels/sparse.py`)** — key contracts for frontends and future agents:
  ...
  - **`sparse_coo_to_csc`** honors the inference-declared indptr length `rows+1` (true column pointers in `ip[0:cols+1]` + monotone nnz tail); `cols > rows` ⇒ explicit `ShapeError` naming the upstream `infer_sparse_coo_to_csc` amendment (should declare `dense_shape[1]+1`). `_csc_to_coo` accepts both the standard `cols+1` and the padded `rows+1` conventions.
