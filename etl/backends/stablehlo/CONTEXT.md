...
- **iree-cuda while-loop fragility is shape-specific (upstream compiler/runtime)**: `while_fib` segfaults and `while_cond_combo` hits a `hal.device.queue.dealloca` ABORT on iree-cuda (both pass on llvm-cpu), while an ISOLATED NSGA2-`non_dominate_rank`-shaped while_loop (3 carries incl. an input-derived vector carry, `etl.select` on carried vectors) compiles and runs bit-exact on BOTH llvm-cpu and iree-cuda (maxdiff 0 / 1.1e-16). However, the FULL NSGA2 environmental-selection defn graph (nsga2.py `_make_tell` — that while_loop-based ranking combined with gather/scatter/sort) makes `iree-compile` itself SEGFAULT (CompilerToolError "Error code: -11", SIGSEGV) on BOTH llvm-cpu and cuda, at every tested pop/dim config — a compiler-side crash, NOT a clean deferred-op `BackendError`, and specific to the COMBINED graph (the isolated loop compiles fine). NSGA2 GPU is therefore blocked on the full tell graph pending an investigation of the while_loop + gather/scatter/sort interplay in the exported module.
