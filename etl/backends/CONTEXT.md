...
| `adapters` | `adapters/__init__.py` | subpackage marker, docstring-only (no heavy imports): documents the three adapter modules `iree.py`/`xla.py`/`tvm.py` (singletons `iree_backend`/`xla_backend`/`tvm_backend` — implemented as a SEPARATE parallel effort, not present here), each module's `register()` contract, the heavy-import rule, and the register-on-first-use flow via `registry.get` |
...
| `./compiler.py` | shared pluggable-compiler framework: `CompilerBackend` (shared `lower`; abstract `check_available`/`compile`/`load`), `CompilerExecutable` (shared save/load; abstract `run`) |
