...
| `./adapters/` | optional compiler adapter modules — ALL IMPLEMENTED: `iree.py` (iree-base-compiler/runtime, VM flatbuffer), `xla.py` + `xla_util.py` (XLA via jaxlib PJRT — embedded CPU client, StableHLO parse → `compile_and_load`), `tvm.py` + `tvm_util.py` (Relax `from_stablehlo` + VM build/run + vendored-translator compat shim); `__init__.py` is docstring-only, no heavy imports |
