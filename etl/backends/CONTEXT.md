...
| Name | Where | Notes |
|---|---|---|
| `Capabilities` | `backend.py` | frozen dataclass: `dynamic_shapes: bool`, `dtypes: frozenset`, `collectives: bool`, `runtime_calls: bool`, `custom_blocks: bool`, `async_collectives: bool`, `sparse_ops: bool`, `rng_bit_generator: bool` (`False` default — a `True` target emits NATIVE `stablehlo.rng_bit_generator` for the threefry2x32/philox4x32_10 random algorithms; `False` targets use the exporter's bit-exact inline expansions — per-adapter values: numpy False, iree False (empiric — see Known Issues), xla True (by design, re-validated with a real PJRT plugin), tvm False) |
