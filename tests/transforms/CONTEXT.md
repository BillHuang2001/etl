...
- `test_vectorize.py` — the batching primitive: per-op numerics vs stacked per-row runs (add/mul/dot/sum), dict/tuple `in_axes` with unmapped broadcast, result contains NO `vectorize` op, input graph not mutated, leading `Dim('batch')` propagation, validation errors (bare callable → TypeError pointing at `etl.vmap`; non-leading/out-of-range axes; pytree mismatch; bare axes with multiple tensor inputs).
