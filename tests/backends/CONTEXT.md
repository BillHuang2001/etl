...
| `test_symbolic_dims.py` | one-build/multi-size symbolic dims (B=3→5), DimExpr reshapes (B*2,), two symbolic dims transpose, None dims, determinism, mixed symbolic+concrete; incl. the known `# BUG(etl)` test (see below) | 8 |
...
- `test_symbolic_dims.py::test_mixed_symbolic_concrete_size_mismatch_raises_etl_error` — FAILS (intentional `# BUG(etl)`). A symbolic dim binding to a size conflicting with a concrete-spec'd input (B=4 vs (3,)) lets a raw `builtins.ValueError` ("operands could not be broadcast together...") escape `etl.run` instead of `etl.ShapeError`, violating the root error-strategy contract (all public errors derive from ETLError). Minimal repro: build `fn(x, w) -> x * w` with specs `TensorSpec((etl.dim("B"),), f32), TensorSpec((3,), f32)`, run with `np.ones(4)`, `np.ones(3)`; the raise site is `etl/backends/numpy/kernels/elementwise.py:81` (numpy broadcast failures not wrapped). Fix belongs in the numpy interpreter/kernels.
