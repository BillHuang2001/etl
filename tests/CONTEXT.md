...
- Plain `python3 -m pytest` is fully green except the known pending adapter-spec items in `## Known Issues` (torch-interop tests skip when torch is absent); any OTHER failure is a regression in either etl or the tests.

## Known Issues

- **Parallel etl implementation still in flight (remaining forward-spec failures):** the pluggable compiler framework + adapters have landed (`etl/backends/compiler.py`, `etl/backends/adapters/{iree,xla,tvm}.py`, lazy adapter loading + install hints in `etl/backends/registry.py`) and `test_adapters_optional.py` is green. 20 failures remain in `tests/backends/`, all pinning fixes still in flight on the etl side: `test_compiler_framework.py` (14) — the test stub cannot instantiate because `CompilerBackend.check_available` is still abstract (fix in flight: make it concrete); `test_adapter_{iree,xla,tvm}.py::test_lower_name_resolution_and_payload_contract` (3) — `payload["entry_functions"]` is still a tuple `('main',)` instead of a list (fix in flight: entry_functions list); `test_adapter_{iree,xla,tvm}.py::test_symbolic_dims` (3) — the dynamic-shape pre-check in `lower` is missing (iree/tvm attempt dynamic lowering and die on the StableHLO parse with `broadcast_in_dim ... tensor<?xf32>`; xla does not raise). These are intentional contract pins — do NOT weaken, skip-guard, or delete them; they are the executable spec the implementation must satisfy. Once the in-flight fixes land and everything is green, DELETE this entry.
