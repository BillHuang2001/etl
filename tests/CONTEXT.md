...
- **Inventory (HEAD e1b9042, `python3 -m pytest --collect-only -q tests/`):** 4854 tests collected across 97 files — backends 604, bench 137, block 116, core 486, dist 242, ir 767, numpy 273, ops 1181, persist 156, pipeline_test 19, sparse 358, test_spec_compliance 79, trace 180, transforms 256. `backends/test_adapter_xla.py` (10) skips at collection without a user-provided PJRT plugin. Run-time skips in the default env (torch absent; iree/tvm/jaxlib/gcc present): 7 torch-interop (6 `core/test_tensor.py::TestTorchInterop` + 1 `test_spec_compliance.py`), 4 `etl.bench` torch-present (`test_torch_repeat_calls.py` ×3 + `test_conformance.py` ×1), 6 xla cases of `backends/test_mixed_dtype.py` → 17. With torch installed the 7+4 run and the 2 torch-absent inverse `etl.bench` tests skip instead; with a PJRT plugin the 16 xla tests run.
