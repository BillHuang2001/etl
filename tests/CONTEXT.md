...
- **Pending parallel etl implementation (forward-spec tests):** `tests/backends/test_compiler_framework.py` and `tests/backends/test_adapter_{iree,xla,tvm}.py` are forward-spec tests for the pluggable compiler backends (`etl/backends/compiler.py`, `etl/backends/adapters/{iree,xla,tvm}.py`, lazy adapter loading + install hints in `etl/backends/registry.py`), being implemented in a parallel change NOT yet landed at HEAD. Until it lands, the suite reports 4 collection errors (those files import `etl.backends.compiler`/`etl.backends.adapters`) plus 4 failures in `test_adapters_optional.py` (registry has no lazy adapter loading / install hint yet). These are intentional contract pins — do NOT weaken, skip-guard, or delete them; they are the executable spec the implementation must satisfy. Once the implementation lands and everything is green, DELETE this entry.
