...
- `backends`: `Backend` interface (`lower`/`compile`/`load`/capabilities), registry, numpy interpreter backend (default, CPU), stablehlo exporter (MLIR text). IREE/XLA/TVM are documented integration points for future optional adapters — not implemented in v1.
