...
Implemented and tested. The full explicit pipeline (`trace → lower → compile → load → run` plus `bind`/`build`/`evaluate` sugar), graph transforms (`vectorize`/`vmap`/`grad`/`jvp`/`vjp`), explicit collectives (`dist`), custom blocks (`block`), the `etl.numpy` graph namespace, the numpy interpreter backend (default, CPU), the StableHLO exporter, the pluggable compiler adapters (IREE — CPU and CUDA validated end-to-end: `target_backends=["cuda"]` compile option + `Device("cuda", N)` load; XLA via the PJRT C API — a ctypes driver over a user-provided plugin `.so`, no jax/jaxlib, plumbing validated via a compiled test-plugin, real-XLA parity pending a real plugin; TVM with the `jax` package removed — jaxlib only for its bundled MLIR bindings — `etl.backends.adapters`, lazy optional extras, CPU-validated end-to-end with numpy parity where the compiler is present), and persistence/caching (`persist`, Graph save/load) are in place. A standalone conformance & benchmark harness (`etl.bench`) ships in the package: 26 curated example programs in four categories — `micro` (matmul, conv2d ×3, elementwise fusion, softmax, layernorm, mlp, cumsum, attention), `grad` (grad_mlp, grad_mix ≈40-op loss, grad_stopgrad, grad_structural — gradients checked against float64 central finite differences AND PyTorch autograd), `vectorize` (vmap_softmax/layernorm/linear/mlp/attention/grad_mlp + vmap_mlp_large — auto-vectorized graphs compared against manually batched numpy references, incl. vmap∘grad composition), and `large` (dummy transformer block, n-body, matmul_1024, conv2d_large, layernorm_large) — all staged through the explicit pipeline and compared against pure-numpy references (and PyTorch references when torch is available), reporting precision (conformance: max-abs/max-rel vs per-example or global tolerance) and speed (benchmark: best-of-N ms + speedup ratios); CLI via `python -m etl.bench`. The harness supports backend/device selection (`--backend`/`--device`/`--backend-option`) and example/category filtering (`--examples NAME[,NAME|CATEGORY]`). Validated matrix (torch on): numpy/cpu 26/26 conformance PASS exit 0; iree cpu+cuda 21/26 run and PASS — including ALL grad examples, 3 of 6 vectorize examples (vmap_mlp/vmap_grad_mlp/vmap_mlp_large now compile and run through auto-vectorized graphs on compiled backends), the transformer (unblocked by the batched-dot exporter fix), n-body, and the large ops — with 5 documented per-example deferrals (cumsum and grad_structural StableHLO export gaps; vmap_softmax/layernorm/attention blocked by dynamic keepdims-reshape rejection). Per-example tolerances relax only where justified (mlp 1e-4, transformer 1e-3, conv2d_large 2e-4, nbody 2e-5, matmul_1024 5e-4 — fp32 accumulation-order noise; IREE is often closer to fp64 ground truth than the numpy fp32 reference). Torch is strictly optional — imported lazily only inside torch-comparison paths, never at module scope. Test suite (`./tests/`): 4251 tests collected, 0 failures; skips depend on torch presence — 12 without torch (8 torch-interop + 3 `etl.bench` torch-present regression tests + 1 xla-contract module without a PJRT plugin), 3 with torch installed (2 `etl.bench` torch-absent inverse tests + 1 xla-contract module). See `./etl/CONTEXT.md` for the package-level public API contract and `./tests/CONTEXT.md` for the test strategy.
