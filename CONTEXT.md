...
- `backends`: `Backend` interface (`lower`/`compile`/`load`/capabilities), registry (lazy activation of optional adapters), numpy interpreter backend (default, CPU), stablehlo exporter (MLIR text — also the shared lowering input for compiler backends), and pluggable compiler adapters `iree` / `xla` (via PJRT, never the jax frontend) / `tvm` in `etl.backends.adapters` — optional extras, activated by `lower(..., backend="iree"|"xla"|"tvm")`, explicit `pip install etl[...]` errors when absent.
