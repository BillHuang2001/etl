...
**v1 deferrals (intentional, explicit errors — never silent fallback):** control-flow vectorization; `conv` VJP (no transposed-conv op); symbolic-length `scan`; `linspace` / `linalg.inv` / `norm` / `det`; non-constant pad modes; symbolic-bound `arange`; a few StableHLO op exports (`cumsum`, `gather`/`scatter`/`scan`, …).
